...
namespace KinematicCharacterController
{
    ...
    public struct CharacterGroundingReport
    {
        ...
        public void CopyFrom(CharacterTransientGroundingReport transientGroundingReport)
        {
            ...
            if (IsStableOnGround == true && transientGroundingReport.IsStableOnGround)
            {
                Debug.LogError("记录离地一颗。。");
            }
    ...
    /// 增加 last groundPoint,否则只有 Normal 有什么用？？
    /// </summary>
...
#pragma warning restore 0649
        ...
        /// </summary>    
        [Tooltip("可离开中心轴的距离【0，0.5】貌似太胖，这个值没什么用 The distance from the capsule central axis at which the character can stand on a ledge and still be stable")]
        public float MaxStableDistanceFromLedge = 0.5f;
        /// <summary>
        /// Prevents snapping to ground on ledges beyond a certain velocity
        /// </summary>    
        [Tooltip("边缘可滑行的最高速度 Prevents snapping to ground on ledges beyond a certain velocity")]
...
#pragma warning restore 0414 
        ...
        public void UpdatePhase1(float deltaTime)
        {
            ...
            #region Ground Probing(探查 探针） and Snapping（断裂）
            // Handle ungrounding
            if (_solveGrounding)
            {
                if (MustUnground())
        ...
        public void UpdatePhase2(float deltaTime)
        {
            ...
            bool isPosTheSame = false;
            // Handle velocity
            CharacterController.UpdateVelocity(ref BaseVelocity, deltaTime);
            if (MyPlayer.Inst.CtlIK._logFlyHeight != 0)
            {
                pos1 = MyPlayer.Inst.CtlIK._logFlyStartPoint;
            }
            if (MyPlayer.Inst.CtlIK._logFlyHeight != 0)
            {
                if (transform.position != pos1)
                {
                    Debug.LogError("p1 offset");
                    isPosTheSame = true;
                }
            }
            ...
            if (MyPlayer.Inst.CtlIK._logFlyHeight != 0)
            {
                if (transform.position != pos1)
                {
                    Debug.LogError("p x2 offset set trans flag=" + flag);
                    isPosTheSame = true;
                }
            }
            ...
            if (MyPlayer.Inst.CtlIK._logFlyHeight != 0)
            {
                if (transform.position != pos1)
                {
                    Debug.LogError("p x3 offset flag=" +flag);
                    isPosTheSame = true;
                }
            }
            ...
            if (MyPlayer.Inst.CtlIK._logFlyHeight != 0)
            {
                if (transform.position != pos1)
                {
                    Debug.LogError("p x4 offset");
                    isPosTheSame = true;
                }
            }

            if (isPosTheSame == false)
            {
                Debug.LogError("p is The same!!!!!!!!!!! "+ (MyPlayer.Inst.CtlIK._logFlyHeight!=0) +" vec=" + BaseVelocity);
            }
        }
        ...
        public void ProbeGround(ref Vector3 probingPosition, Quaternion atRotation, float probingDistance, ref CharacterGroundingReport groundingReport)
        {
            ...
            {
                //（打扫; 扫; 清扫; 扫去; 清除，吹走，冲走） Sweep for ground detection
                if (CharacterGroundSweep(
                        ...
                        out groundSweepHit)) // hit
                {
                    ...
                    {
                        Debug.LogError("Force Leave Ground");
            ...
            //     Debug.LogError("_solveGrounding after Probe Ground ??" + _solveGrounding +" stable??"+GroundingStatus.IsStableOnGround);
        }
        ...
        {
            //_solveMovmentCollider 即调用这个方法，模拟物理碰撞
            //Debug.LogError("Internal Character Move...");
            Vector3 initVelocity = transientVelocity;
            ...
            // // 打印速度变化
            // if(initVelocity == transientVelocity)
            // {
            //     Debug.LogWarning($"Test <color=red>velocity={initVelocity}->{transientVelocity} 速度={transientVelocity.sqrMagnitude}</color>");
            // }
            // else
            // {
            //     Debug.LogWarning($"Test velocity={initVelocity}->{transientVelocity} 速度={transientVelocity.sqrMagnitude}");    
            // }
