...
namespace KinematicCharacterController.Examples
{
    public class ExampleCharacterCamera : MonoBehaviour
    {
        ...
        /// <summary>
        /// 增加了 pDirection ，需传入控制镜头左右跟随人物；而镜头固定高低，由 pY 模拟控制（之后会根据运动状态微调）
        /// </summary>
        /// <param name="deltaTime"></param>
        /// <param name="zoomInput"></param>
        /// <param name="rotationInput">一个向右|左的力（配合鼠标移动使用而已）</param>
        public void UpdateWithInput(float deltaTime, float zoomInput, Vector3 rotationInput,Nullable<Vector3> pDirection = null,float pX = 27f) 
        {
            if (FollowTransform)
            {
                if (InvertX)
                ...
                if (pDirection != null)
                {
                    PlanarDirection = pDirection.Value;
                ...
                //新滑板 抬头
                Quaternion verticalRot = Quaternion.Euler(pX, 0, 0);
                Quaternion targetRotation = Quaternion.Slerp(Transform.rotation,  planarRot * verticalRot, 1f - Mathf.Exp(-RotationSharpness * deltaTime));
                if (pDirection == null)
                {
                    //原 mouse 抬头
                    verticalRot = Quaternion.Euler(_targetVerticalAngle, 0, 0);//原 mouse 转向
