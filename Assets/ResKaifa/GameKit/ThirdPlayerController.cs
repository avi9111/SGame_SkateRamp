...
/// <summary>
/// 依赖 LowPoly Nature 这个插件耶。。。。。。（不确定哪个场景用到了ThirdPlayerController这个类
/// </summary>
...
public class ThirdPlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        if (!IsDead && mIsControlEnabled)
        {
            ...
            //Vector3 camForward_Dir = Camera.main.transform.forward.normalized;
            ...
            if (_characterController.isGrounded)
            {
                ...
                {
                    //_animatorSetBool("is_in_air", true);
                    _moveDirection.y = JumpSpeed;

                }
                else
                {
                    //_animatorSetBool("is_in_air", false);
                    _animatorSetBool("run", move.magnitude > 0);
                }
            }
    ...
    void _animatorSetBool(string key, bool v)
    {
        if (_animator == null) return;
        //_animatorSetBool(key, v);//？？ 死循环？？？
        
    }
    void _animatorSetTrigger(string triggerName)
    {
        if (_animator == null) return;
        //    _animatorSetTrigger(triggerName);
    }
